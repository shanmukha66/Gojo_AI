...
MODEL_LABEL = os.getenv("MODEL_LABEL", "claimsdb_tabular_xgboost")
...
def main():
    ...
    with open(os.path.join(OUT_DIR, "metrics_claimsdb_tabular.json"), "w", encoding="utf-8") as f:
        json.dump(metrics, f, indent=2)

    with open(os.path.join(OUT_DIR, "curves_claimsdb_tabular.json"), "w", encoding="utf-8") as f:
        json.dump(curves, f, indent=2)

    with open(os.path.join(OUT_DIR, "subgroup_claimsdb_tabular.json"), "w", encoding="utf-8") as f:
        json.dump(subgroups, f, indent=2)

    if shap_features:
        with open(os.path.join(OUT_DIR, "shap_claimsdb_tabular.json"), "w", encoding="utf-8") as f:
            json.dump({"dataset": DATASET_LABEL, "model": MODEL_LABEL, "features": shap_features}, f, indent=2)

    print("Saved ClaimsDB tabular metrics, curves, subgroup, and SHAP artifacts")
