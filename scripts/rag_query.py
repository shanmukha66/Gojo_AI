...
if not query_text:
    raise SystemExit("Usage: rag_query.py <your question>")
...
with driver.session() as session:
    result = session.run(
        ...
        YIELD node, score
        RETURN node.id AS id, node.name AS name, node.age AS age, node.risk AS risk, node.description AS description, score
        ORDER BY score DESC
        """,
...
for row in rows:
    print("\n---")
    print(f"Patient: {row.get('name')} (ID {row.get('id')})")
    print(f"Age: {row.get('age')}  Risk: {row.get('risk')}")
